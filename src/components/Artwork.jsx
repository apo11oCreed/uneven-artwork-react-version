					...
					return (
						<li key={index} className='galleryTwoCols md:galleryThreeCols tw-rounded-lg tw-mt-[1.5rem] tw-overflow-hidden'>
							<figure className='tw-relative tw-w-auto tw-h-[25rem] tw-overflow-hidden tw-flex tw-justify-center tw-items-center tw-flex-col'>
								...
								<figcaption id={`img_${index}`} className='sr tw-relative tw-max-w-[50%] tw-p-[1rem] tw-text-[2rem] tw-leading-[1] tw-text-center'>{item.image.title}</figcaption>
								<button
...
'
			...
			{showModal && (
					<Modal image={modalContent} showModal={setShowModal} activateTrap={setActivateTrap} />
			)}
		</>
	);
}
